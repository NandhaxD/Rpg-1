...
async def create_battle(user_id: int, enemy: int, cq):
  start = time.time()
  ...
  if is_battling:
    return False
      ...
      await db.battle.insert_one({"battle_id": unique_code, "player": user_id, "enemy_id": enemy, "time": time.time(), "message_id": cq.message.id})
      return unique_code
