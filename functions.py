...
async def get_town(call):
    cur_town_id = (await db.persons.find_one({"Nickname": call.from_user.username}))["LocationID"]
    cur_town = (await db.locations.find_one({"LocationID": cur_town_id}))["LocationName"]
    player = await db.persons.find_one({"Nickname": call.from_user.username})
...
async def get_dungeon(call):
    cur_dungeon_id = (await db.persons.find_one({"Nickname": call.from_user.username}))["LocationID"]
